                    ...
                    for i, pollutant in enumerate(pol_list):
                        ...
                        if i == 0:
                            ...
                            if not pollutant.startswith('pm'):
                                ...
                                query += """LEFT JOIN (SELECT trans.total_emissions / {reduction_factor} AS {pollutant}_trans,
                                                              trans.fips                                 AS fips
                                                       FROM   {scenario_name}.transportation trans
                                                       WHERE  trans.feedstock      = '{feed}'
                                                         AND  trans.pollutantID    = '{pollutant}'
                                                   ...
                                                   ON feed_{pollutant}.fips = feed_sox.fips
                                            LEFT JOIN (SELECT fd.total_fd_emissions / {reduction_factor} AS {pollutant}_fug,
                                                              fd.fips AS fips
                                                       FROM   {scenario_name}.fugitive_dust fd
                                                       WHERE  fd.feedstock      = '{feed}'
                                                         AND  fd.pollutantID    = 'pm25'
                                                         AND  fd.logistics_type = 'C'
                ...
                for act_num, activity in enumerate(self.act_list):
                    ...
                    if activity != 'Harvest':
                        query = """SELECT    selected.{pollutant}/total.sum_pol
                                   ...
                                   WHERE     source_category LIKE '%{act}%'
                                     AND     feedstock          = '{feed}'
                                     AND     total.sum_pol      > 0
                        ...
                        query = """SELECT    selected.{pollutant}/total.sum_pol
                                   ...
                                   LEFT JOIN (SELECT   fips,
                                          ...
                                          ON total.fips = selected.fips
                                   WHERE     source_category LIKE '%{act}%'
                                     AND     NOT source_category LIKE '%Non-Harvest%'
                                     AND     feedstockpol = '{feed}'
                                     AND     total.sum_pol > 0
                                   GROUP BY  selected.fips
                                   ;""".format(**kvals)
