...
class FugitiveDust:
    ...
    def __init__(self):
        ...
        budg_dat.loc[(budg_dat['croptype'] == 'R') &
                     (budg_dat['crop'] == 'Corn'), 'crop'] = 'Corn stover'

        budg_dat.loc[(budg_dat['croptype'] == 'R') &
                     (budg_dat['crop'] == 'Sorghum'), 'crop'] = 'Sorghum ' \
                                                                'stubble'

        budg_dat.loc[(budg_dat['croptype'] == 'R') &
                     (budg_dat['crop'] == 'Wheat'), 'crop'] = 'Wheat straw'

        self.budg_dat = budg_dat
...
def set_fugdust_emissions(self):
    ...
    # this sums the hp_hrs values within each group defined by the budget id,
    # then calculates the PM10 allocation factor by dividing each individual
    # hp_hrs value by the budget id group total.
    # tested calculation using BA.CT.01, math checked out
    self.budg_dat['hp_hrs_alloc'] = \
        self.budg_dat.groupby('bdgt_id').hp_hrs.apply(lambda x: x /
                                                                float(x.sum()))
    ...
    budg_dat_harv = self.budg_dat.loc[self.budg_dat.opertype.isin(('HRVR',
                                                                   'HRVC'))]
    budg_dat_nonharv = self.budg_dat.loc[self.budg_dat.opertype.isin(('ESTC',
                                                                      'MNTR',
                                                                      'MNTC'))]

    # similarly divide the fugdust_ef into harvest and non-harvest data frames
    fugdust_ef_harv = self.fugdust_ef.loc[self.fugdust_ef.source_category ==
                                          'Harvest']
    fugdust_ef_nonharv = self.fugdust_ef.loc[self.fugdust_ef.source_category ==
                                             'Non-Harvest']
    ...
    # harvest and non-harvest PM10 and PM2.5 emissions (pounds per acre) and
    # the silt table with the silt information necessary to calculate PM10 and
    # PM2.5 from on-farm travel
    self.output_df = self.prod_dat.merge(self.budg_dat_fugdust,
    ...
    # harvested acres and adding PM10 from on-farm travel (also allocated
    # between opertypes by horsepower-hours), then converting the sum to
    # metric tons
    self.output_df['pm10'] = (self.output_df.harv *
                              self.output_df.pm10_lbac_alloc) * \
    ...
    self.output_df['pm25'] = (self.output_df.harv *
                              self.output_df.pm25_lbac_alloc) * \
                             self.convert_lb_to_mt
